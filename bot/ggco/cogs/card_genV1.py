...
async def card_generator(ctx, user, client):
    ...
    background = Image.open(r"bot/assets/images/background/resumeBackground.png")
    ...
    if (
        guild.get_role(roles_config.unit_roles["tanks"])
...
class Card(commands.Cog):
    ...
    @commands.command()
    async def card1(self, ctx, user: discord.Member = None):
        await card_generator(ctx, user, self.client)
