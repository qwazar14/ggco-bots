...
class RegistrationMenu(commands.Cog):
    ...
    async def registration_menu(self, ctx):
        class RegistrationMenuButtons(discord.ui.View):
            ...
            @discord.ui.button(label='Подать заявку в полк', style=discord.ButtonStyle.green)
            async def join_squadron(self, button, ctx, timeout_error_call=True, guild_id=guild_id):
                ...
                user = ctx.user
                new_nickname = ''
                await ctx.response.send_message(content='Введите ник в игре *(у вас есть 1 минута)* ',
                                                ephemeral=True)
                try:
                    nickname_user = await get_user_response(self.client, ctx)
                    await ctx.edit_original_message(content='Как Вас зовут? *(у вас есть 1 минута)*')
                    ...
                    try:
                        ...
                        if timeout_error_call is not False:
                            if 1.0 <= br_user <= settings['maxBR']:
                                ...
                                timeout_error_call = False
                            if timeout_error_call is not False:
                                await finish_registration_if_ok(user, guild_id, new_nickname, True)
                                await ctx.edit_original_message(content='*Регистрация завершена*')

                    except asyncio.TimeoutError:
                        await timeout_error(ctx)

            @discord.ui.button(label='Другой полк', style=discord.ButtonStyle.blurple)
            async def squadron_friend(self, button, ctx, timeout_error_call=True, guild_id=guild_id):
                user = ctx.user
                await ctx.response.send_message(content='Введите ник в игре *(у вас есть 1 минута)* ',
                                                ephemeral=True)
                try:
                ...
                if timeout_error_call is not False:
                    ...
                    class SquadronMenu(discord.ui.View):
                        ...
                        async def get_user_squadron(self, button, interaction):
                            ...
                            await interaction.response.send_message(content='*Введите клантег без \"рамок\"',
                                                                    ephemeral=True)
                            try:
                                squadron_user = await get_user_response(self.client, interaction)
                        ...
                        async def end_user_registration(self, button, interaction):
                            ...
                            await user.edit(nick=new_nickname)
                            await finish_registration_if_ok(user, guild_id, new_nickname, False)
                            await edit_final_message_to_finish_registration(self, interaction)

                    if timeout_error_call is not False:
                        view_squadron_buttons = SquadronMenu(self.client)
                        await ctx.edit_original_message(content='*Вы состоите в полку?*',
        ...
        embed.add_field(name="Если вы хотите зайти на сервер - нажмите кнопку 'Друг полка'",
...
async def get_user_response(client, interaction):
    msg = await client.wait_for("message", timeout=60, check=lambda
...
async def give_user_friend_roles(user, guild_id):
    await user.add_roles(guild_id.get_role(roles_config.optional_category_roles['squadron_friend']))
...
async def give_user_basic_roles(user, guild_id):
    ...
    await user.add_roles(guild_id.get_role(roles_config.roles_categories['optional_category']))
    await user.add_roles(guild_id.get_role(roles_config.roles_categories['general_category']))
