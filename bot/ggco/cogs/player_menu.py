...
class PlayerMenu(commands.Cog):
    def __init__(self, client):
        self.client = client

    # @commands.command(pass_context=True)
    @commands.has_any_role(roles_config.discord_roles['admin'])
    @commands.slash_command(name="br_menu", description='Создает меню регистрации', guild_ids=[398857722159824907])
    async def player_menu(self, ctx):
        class PlayerMenuButtons(discord.ui.view):
            ...
            @discord.ui.button(label='Запросить повышение', style=discord.ButtonStyle.green)
        ...
        buttons = PlayerMenuButtons(self.client)
