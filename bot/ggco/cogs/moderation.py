...
class Moderation(commands.Cog):
    ...
    @commands.command(
        usage="clear [лимит]", description="Очистить сообщения", brief="clear 10"
    )
    @commands.has_any_role(roles_config.discord_roles["admin"])
    async def clear(self, ctx, limit: int):
        try:
            await ctx.channel.purge(limit=limit + 1)
            embed = discord.Embed(description=f"Очищено {limit} сообщ.", color=0xE871FF)
    ...
    async def remwarn(self, ctx, id: int):
        with self.con.cursor() as cursor:
            cursor.execute(
                "SELECT gid, uid FROM `WarnsDB` WHERE `gid`=%s AND `id`=%s",
                (ctx.guild.id, id),
            )
            if cursor.fetchone() == None:
                embed = discord.Embed(
                    description=f"Данного ID нету в базе!", color=ctx.author.color
                )
                await ctx.send(embed=embed)
            else:
                cursor.execute(
                    "DELETE FROM `WarnsDB` WHERE `id`=%s AND `gid`=%s",
                    (id, ctx.guild.id),
                )
                embed = discord.Embed(
                    description=f"Предупреждение **{id}** было удалено", color=0xE871FF
                )
    ...
    async def resetwarns(self, ctx, user: discord.Member):
        with self.con.cursor() as cursor:
            ...
            if cursor.fetchone() == None:
                ...
                cursor.execute(
                    "DELETE FROM `WarnsDB` WHERE `uid`=%s AND `gid`=%s",
    ...
    async def warns(self, ctx, user: discord.Member = None):
        if user is None:
            ...
            pass
        embed = discord.Embed(
            description=f"Предупреждения участника {user} ({user.id})", color=0xE871FF
        )

        with self.con.cursor() as cursor:
            cursor.execute(
                "SELECT gid, uid FROM `WarnsDB` WHERE `gid`=%s AND `uid`=%s",
                (ctx.guild.id, user.id),
            )
            if cursor.fetchone() == None:
                embed.add_field(
                    name=f"Предупреждений не найдено", value=f"** **", inline=False
                )
        self.con.commit()

        with self.con.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM `WarnsDB` WHERE `gid`=%s AND `uid`=%s",
                (ctx.guild.id, user.id),
            )
    ...
    @commands.has_any_role(roles_config.discord_roles["admin"])
    async def mute(
        self, ctx, user: discord.Member, duration, *, reason="Причина не указана"
    ):
        with self.con.cursor() as cursor:
            if user == ctx.author:
                embed = discord.Embed(
                    description=f"Ты не можешь заглушить самого себя!", color=0xFA4C4D
                )
                await ctx.send(embed=embed)
                ...
                if duration.endswith("s"):
                    ...
                    await ctx.send(
                        content="Неправильный формат времени", delete_after=5
                    )

                await user.add_roles(role)
                embed = discord.Embed(
                    title=f"✅ | Участник {user.name} замьючен! 🙊", color=0xE871FF
                )
    ...
    async def unmute(self, ctx, user: discord.Member):
        if user == ctx.author:
            embed = discord.Embed(
                description=f"Ты не можешь размутить самого себя!", color=0xFA4C4D
            )
            await ctx.send(embed=embed)
        elif ctx.author.top_role.position < user.top_role.position:
            embed = discord.Embed(
            ...
            embed = discord.Embed(description=f"Размут", color=0xE871FF)
            embed.add_field(name="Модератор", value=ctx.author.mention)
    ...
    async def kick(self, ctx, user: discord.Member, *, reason="Причина не указана"):
        if user == ctx.author:
            embed = discord.Embed(
                description=f"Ты не можешь кикнуть самого себя!", color=0xFA4C4D
            )
            await ctx.send(embed=embed)
        elif user == ctx.guild.owner:
            embed = discord.Embed(
                description=f"Ты не можешь кикнуть владельца сервера!", color=0xFA4C4D
            )
            await ctx.send(embed=embed)
        elif ctx.author.top_role.position < user.top_role.position:
            embed = discord.Embed(
            ...
            embed = discord.Embed(description=f"Кик с сервера", color=0xE871FF)
            embed.add_field(name="Модератор", value=ctx.author.mention)
            embed.add_field(name="Участник", value=user)
            await ctx.send(embed=embed)
    ...
    async def ban(self, ctx, user: discord.Member, *, reason="Причина не указана"):
        if user == ctx.author:
            embed = discord.Embed(
                description=f"Ты не можешь забанить самого себя!", color=0xFA4C4D
            )
            await ctx.send(embed=embed)
        elif user == ctx.guild.owner:
            embed = discord.Embed(
                description=f"Ты не можешь забанить владельца сервера!", color=0xFA4C4D
            )
            await ctx.send(embed=embed)
        elif ctx.author.top_role.position < user.top_role.position:
            embed = discord.Embed(
            ...
            embed = discord.Embed(description=f"Бан", color=0xE871FF)
            embed.add_field(name="Модератор", value=ctx.author.mention)
            embed.add_field(name="Участник", value=user)
            await ctx.send(embed=embed)
    ...
    async def unban(self, ctx, user_id: int):
        ...
        for ban_entry in banned_users:
            ...
            if user == ban_user:
                ...
                embed = discord.Embed(description=f"Разбан", color=0xE871FF)
                embed.add_field(name="Модератор", value=ctx.author.mention)
                embed.add_field(name="ID Участник", value=user)
                await ctx.send(embed=embed)
                return
        embed = discord.Embed(
            description=f"***{user}*** не забанен на данном сервере", color=0xFA4C4D
        )
        await ctx.send(embed=embed)
