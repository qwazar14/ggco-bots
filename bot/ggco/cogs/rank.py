...
class RankManager(commands.Cog):
    ...
    @commands.slash_command(name="down", description='Запрашивает понижение конкретного игрока в звании на 1', guild_ids=[398857722159824907])
    async def down(self, ctx, user: discord.Member):

        class View(discord.ui.View):

            @discord.ui.select(placeholder='Выберите ранг', min_values=1, max_values=1,
            ...
            async def confirm(self, button, interaction):
                ...
                if not RankSystem.if_rank_member1_above_member2(interaction.user, ctx.author):
                    ...
                    return'''
                await ctx.author.remove_roles(ctx.guild.get_role(ranks_controller.get_rank_id_by_name(user_rank)))
                await ctx.author.add_roles(ctx.guild.get_role(ranks_controller.get_rank_id_by_name(self.down_rank)))
        ...
        embed.add_field(name=f"Вы собираетесь понизить {ctx.author.nick} в ранге",
    ...
    @commands.slash_command(name="up", description='Запрашивает повышение вызвавшего игрока в звании на 1', guild_ids=[398857722159824907])
    async def up(self, ctx):
        ...
        class View(disnake.ui.View):
            ...
            async def rank_up(self, button, interaction):
                ...
                if interaction.user == ctx.author:
                    await interaction.response.send_message(content='Вы не можете повысить самого себя!',
                                                            ephemeral=True)
                    return
                if new_rank in ranks_controller.get_officers_ranks_id():
                    if not ranks_controller.if_member_can_up_officers(interaction.user):
                        await interaction.response.send_message(content='Вы не можете повышать офицеров!',
                                                                ephemeral=True)
                        return
                if not ranks_controller.if_rank_member1_above_member2(ctx.author, interaction.user):
                    await interaction.response.send_message(
                        content='Вы не можете повышать игроков, у которых ранг выше вашего!', ephemeral=True)
                    return
                await ctx.author.remove_roles(ctx.guild.get_role(ranks_controller.get_rank_id_by_name(rank)))
            ...
            async def deny(self, button, interaction):
                ...
                if not ranks_controller.if_member_can_up_officers(interaction.user):
                    ...
                    return
                if not ranks_controller.if_rank_member1_above_member2(interaction.user, ctx.author):
                    await interaction.response.send_message(
                        content='Вы не можете повышать игроков, у которых ранг выше вашего!', ephemeral=True)
                    return
        ...
        embed.add_field(name=f"Игрок {ctx.author.nick} запрашивает повышение.",
