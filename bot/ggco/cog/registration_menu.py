...
class RegistrationMenu(commands.Cog):
    ...
    async def registration_menu(self, ctx):
        class RegistrationMenuButtons(disnake.ui.View):
            ...
            @disnake.ui.button(label='Подать заявку в полк', style=disnake.ButtonStyle.green)
            async def join_squadron(self, button, interaction, timeout_error_call=True, guild_id=guild_id):
                ...
                await interaction.response.send_message(content='Введите ник в игре *(у вас есть 30 секунд)* ',
                                                        ephemeral=True)
                try:
                    nickname_user = await get_user_response(self.client, interaction)
                    await interaction.edit_original_message(content='Как Вас зовут? *(у вас есть 30 секунд)*')
                except asyncio.TimeoutError:
                    timeout_error_call = await timeout_error(interaction)

                if timeout_error_call is not False:
                    try:
                        name_user = await get_user_response(self.client, interaction)
                ...
                if timeout_error_call is not False:
                    try:
                        br_msg_content = await get_user_response(self.client, interaction)
                        try:
            ...
            @disnake.ui.button(label='Друг полка', style=disnake.ButtonStyle.blurple)
            async def squadron_friend(self, button, interaction, timeout_error_call=True):
                ...
                if timeout_error_call is not False:
                    ...
                    class SquadronMenu(disnake.ui.View):
                        def __init__(self, client, *, timeout=None):
                            super().__init__(timeout=timeout)
                            self.client = client
                        ...
                        async def end_user_registration(self, button, interaction):
                            ...
                            await user.edit(nick=new_nickname)
                            # await interaction.edit_original_message(content='*Регистрация завершена*')
                            await end_registration(self, interaction)
        ...
        embed = disnake.Embed(title="Вы попали на сервер полка GG Company",
...
async def get_user_response(client, interaction):
    msg = await client.wait_for("message", timeout=30, check=lambda
