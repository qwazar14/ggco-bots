...
async def on_voice_state_update(member, before, after):
    ...
    if after.channel is not None:
        if (
            after.channel.id != settings["squadron_battle_channel_1"]
            ...
            if after.channel.id == settings["squadron_battle_channel_1"]:
                ...
                await member.remove_roles(
                    guild.get_role(channels_roles["waiting_role"])
                )
...
async def on_slash_command_error(interaction, exception):
    await interaction.response.send_message(
        content="У вас недостаточно прав для вызова этой команды! Воспользуйтесь /help",
        ephemeral=True,
    )
...
async def on_command(ctx):
    print(
        f"[INFO] {ctx.author} called command {ctx.command}:\nArgs: {ctx.args}\nKwargs: {ctx.kwargs}"
    )


@commands.has_any_role(discord_roles["admin"])
@client.slash_command(
    name="reload", description="Перезагружает модули", guild_ids=[398857722159824907]
)
...
@client.slash_command(
    name="unload", description="Отключает модули", guild_ids=[398857722159824907]
)
...
@commands.has_any_role(roles_config.discord_roles["admin"])
@client.slash_command(
    name="load", description="Загружает модули", guild_ids=[398857722159824907]
)
# @client.command()
...
async def load_all_cogs():
    embed = discord.Embed(title=f"**LOADING ALL COGS**", color=0xE100FF)
    ...
    for filename in os.listdir("../ggco/cogs"):
        if filename.endswith(".py") and filename != "db.py":
            try:
                ...
                embed.add_field(
                    name=f"{filename}", value=f"```css\n[{exception}]```", inline=False
                )

                print(f"[ERROR] load {filename}: {exception}")
            else:
                embed.add_field(
                    name=f"{filename}", value=f"```ini\n[loaded!]```", inline=True
                )
                print(f"...{filename[:-3]} loaded!")

    embed.add_field(
        name=f"```{time.strftime('%d %b %Y')}```",
        value=f"```fix\n{time.strftime('%H:%M:%S')}```",
        inline=False,
    )
...
async def user_join_left_controller(member, mode):
    if mode:
        embed = discord.Embed(
            title=f"{member} зашёл на сервер", color=settings["okColor"]
        )
    else:
        embed = discord.Embed(
            title=f"{member} покинул сервер", color=settings["noOkColor"]
        )
    embed.add_field(name="\u200b", value=f"Ник: {member.mention}")
    embed.add_field(name="\u200b", value=f"Время: {time.strftime('%H:%M:%S %d %b %Y')}")
    try:
...
async def give_user_basic_roles(member):
    role = discord.utils.get(
        member.server.roles, id=roles_config.general_category_roles["new_player"]
    )
